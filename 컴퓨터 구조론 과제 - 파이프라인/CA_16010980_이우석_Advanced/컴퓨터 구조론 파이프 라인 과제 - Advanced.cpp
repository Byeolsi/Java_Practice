...
int main(void)
{
	...
	while (pc < 64)
	{
		...
		add_result = Add(pc_add_4, inst_ext_shift);
		...
		mux_result = Mux(control.Branch & alu.zero, pc_add_4, add_result);

		mux_result = Mux(control.Jump, mux_result, jump_addr);
		...
		mem_result = Mux(control.MemtoReg, alu.ALU_result, mem_result);
